...
QEMU (tracking `master` of the `milesbuckton/qemu` fork) with **VirGL/ANGLE** OpenGL acceleration **and** **Venus** Vulkan
...
shadows the core `qemu`/`libepoxy` kegs). Either invoke QEMU via its opt path:
...
> Apple Silicon are published on each release; the `publish.yml` workflow builds and smoke-tests
> them on macOS.
...
- `Formula/` — `qemu-virgl.rb` (tracks `master` of the `milesbuckton/qemu` fork) plus supporting formulae `libangle` (latest `main`), `libepoxy-angle` (latest `master`), `virglrenderer` (tracks `main` of the `miles.buckton/virglrenderer` fork), and `gn` (latest `master`, build-only tool dependency of libangle). The QEMU and virglrenderer formulae track their respective forks rather than upstream.
