...
class Libangle < Formula
  ...
  url "https://chromium.googlesource.com/angle/angle.git",
      revision: "3e88857d92eb32f2c862652bbf8603d97f3789b0",
      using:    NoSubmoduleGitDownloadStrategy
  version "main.20260909.3e88857d9"
  license "BSD-3-Clause"

  bottle do
    root_url "https://github.com/milesbuckton/homebrew-qemu-virgl/releases/download/latest"
    rebuild 1
    sha256 cellar: :any, arm64_tahoe: "955ceed0661a5fec8c06d6b8f6d3beb9a39aa1c6cb2c7ee472ed18d382ba15a7"
  end
  ...
  def install
    ...
    # Chromium's bundled clang. Tested against ANGLE revision 62861200bf
    # (version "2025.11.24"). Adjust if upstream toolchain.gni changes.
    ...
    inreplace "build/toolchain/apple/toolchain.gni" do |s|
      ...
      s.gsub!('_installnametoolpath = "${prefix}llvm-install-name-tool"',
'_installnametoolpath = "install_name_tool"')
      s.gsub!(%r{rebase_path\("//tools/clang/dsymutil/bin/dsymutil",\s+root_build_dir\)}, '"dsymutil"')
    end
    ...
    ln_s Formula["python@3.14"].opt_bin/"python3",
    ...
    if File.exist?("gni/angle.gni") && File.read("gni/angle.gni").include?("sanitize_c_array_bounds")
      ...
      inreplace "gni/angle.gni", %r{# See https://crbug.com/386992829.*?^  \}}m, ""
    end
